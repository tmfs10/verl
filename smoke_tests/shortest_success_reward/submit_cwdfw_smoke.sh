...
# One-update, dry-run-first CW-DFW smoke for the grouped shortest-success
# training reward. Validation deliberately remains on raw verifier accuracy.
...
VERIFY_SCRIPT="${VERIFY_SCRIPT:-$VERL_ROOT/smoke_tests/shortest_success_reward/verify_reward_audit.py}"
CONFIG_DIR="${CONFIG_DIR:-/home/siddjain/workspace/scripts/nemo_configs/cluster/codegen}"

RUN_TAG="${RUN_TAG:-$(date +%Y%m%d_%H%M%S)}"
MARGIN_PERCENT="${MARGIN_PERCENT:-10.0}"
...
EXP_NAME="shortest_success_qwen3_1p7b_smoke_${RUN_TAG}"
OUTPUT_BASE="${OUTPUT_BASE:-/output/smoke_tests/shortest_success_reward_${RUN_TAG}}"
...
LOCAL_RUN_DIR="${LOCAL_RUN_DIR:-/home/siddjain/data/smoke_tests/shortest_success_reward/$RUN_TAG}"
...
PY
  ...
  # The launcher's dry-run deliberately stages the exact VERL tree into
  # REMOTE_EXP_DIR/verl_code. Permit that staging on the subsequent submit,
  # while refusing to reuse a directory that already contains run outputs.
  ...
  echo "[preflight] train_rows=$rows train_sha256=$sha val_rows=$val_rows val_batches=1 output=$REMOTE_EXP_DIR"
}
...
++reward.reward_kwargs.use_shortest_success_reward=true
++reward.reward_kwargs.shortest_success_margin_percent=$MARGIN_PERCENT
++reward.reward_kwargs.shortest_success_threshold=0.5
EOF
}
...
submit() {
  ...
  if [[ "$dry_run" == 1 ]]; then
    action_log="$LOCAL_RUN_DIR/dry_run.log"
  else
...
verify() {
  ...
  mkdir -p "$generations_dir" "$report_dir"
