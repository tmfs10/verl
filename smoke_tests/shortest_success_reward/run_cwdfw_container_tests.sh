...
# Run the focused reward-manager suite in the exact live VERL container.
...
REMOTE_STAGE_BASE="${REMOTE_STAGE_BASE:-/lustre/fsw/portfolios/llmservice/users/siddjain/nemo-run/output/setup/shortest_success_reward}"
...
LOCAL_LOG_DIR="${LOCAL_LOG_DIR:-/home/siddjain/data/smoke_tests/shortest_success_reward/container_tests_$RUN_TAG}"
...
python -m pytest -q \
  tests/trainer/ppo/test_resume_controls.py \
...
ssh "$SSH_TARGET" bash -s -- \
  "$ACCOUNT" "$PARTITION" "$TIME_LIMIT" "shortest-reward-tests-$RUN_TAG" \
