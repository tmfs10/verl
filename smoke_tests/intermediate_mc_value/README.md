...
| Two-GPU dynamic update parity | `torchrun --standalone --nproc-per-node=2 --module smoke_tests.intermediate_mc_value.dynamic_critic_update_smoke --strategy {fsdp,fsdp2} --output-json ...` (run automatically by `run_smoke.sh`) | Unequal per-rank token workloads begin with two versus one dynamic microbatches, synchronize to unequal two-row/three-row microbatches under NCCL, execute real scalar and Beta forward/backward/optimizer steps with remove-padding, and match fixed-microbatch critic losses and parameter updates under legacy FSDP and FSDP2. Equal per-row target cardinality makes this a valid parity invariant for VeRL's actor-native row-fraction scaling; heterogeneous target construction is covered separately by CPU orchestration tests. |
...
By default the runner first executes a two-GPU fixed-versus-dynamic critic
update parity preflight for scalar and Beta heads under both legacy `fsdp` and
`fsdp2`. It then executes twelve core end-to-end cells: `scalar_random`,
`scalar_ema`, and `beta_variance`, each under both backends, once with two
critiques and once with `num_critiques=0`. Two additional `scalar_random`
end-to-end cells enable dynamic batching only for the critic, leaving the actor
on its native fixed microbatches. All generation and validation use temperature
`1.0`; logging is console-only and `WANDB_MODE=disabled`.
...
```bash
CELLS=beta_variance DYNAMIC_CELLS=scalar_random BACKENDS=fsdp2 \
  CRITIQUE_COUNTS="2 0" RUN_RESUME=1 \
  bash smoke_tests/intermediate_mc_value/run_smoke.sh
```

Set `RUN_DYNAMIC_PARITY=0` to skip the dedicated two-GPU parity preflight, or
set `DYNAMIC_CELLS=""` to omit the focused dynamic end-to-end cells. Those
options reduce coverage and are intended only for targeted reruns.
